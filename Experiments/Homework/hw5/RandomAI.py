...
class Evaluate(object):
    ...
    def evaluate(self, chess_map):
        point = 0
        for chess in chess_map.get_chess():
...
class ChessMap(object):
    ...
    def get_chess(self):
        """获取所有的棋盘上的棋子对象列表"""
        return [chess for line in self.chess_map for chess in line if chess]

    def virtual_move_chess(self, old_row, old_col, new_row, new_col):
        # 与ChessBoard.move_chess()不同, 这只是在棋盘拷贝上移动棋子, 不会改变原来的棋盘
        # 移动位置
        self.chess_map[new_row][new_col] = self.chess_map[old_row][old_col]
        # 修改棋子的属性
        self.chess_map[new_row][new_col].update_position(new_row, new_col)
        # 清楚之前位置为None
        self.chess_map[old_row][old_col] = None
